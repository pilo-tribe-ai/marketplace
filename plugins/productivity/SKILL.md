...
- **`wrapping-up-sessions`** — tear down a finished session's workspace: verify nothing is uncommitted/unpushed, confirm the work reached the default branch, then delete the local branch and remove the worktree it runs in. Outside a worktree it returns to the default branch and pulls. It never touches the main checkout.
