#!/usr/bin/env bash
# Thin claude-session driver. Runs as a child subprocess of acpx-dispatch.sh.
...
# Emits BLOCKED envelope and exits 1 — matching codex-session-driver.sh.
...
# Ensure session — idempotent, re-run-safe. Session is scoped by (engine, cwd, name);
# every subsequent subcommand passes the identical --cwd.
