...
The diagrams carry the weight. Prose is sparse, plain, and uses the glossary terms (from the `/engineering:codebase-design` skill) without ceremony.
...
Plain English, concise, but the architectural nouns and verbs come straight from the `/engineering:codebase-design` skill. Concision is not an excuse to drift.
...
No hedging, no throat-clearing, no "it's worth noting that…". If a sentence could be a bullet, make it a bullet. If a bullet could be cut, cut it. If a term isn't in the `/engineering:codebase-design` glossary, reach for one that is before inventing a new one.
